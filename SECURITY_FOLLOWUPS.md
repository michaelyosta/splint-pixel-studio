...
| SEC-003 | high | open | `PUT /colorings/:id/progress` в `server/routes/colorings.js` принимает полный `filled`; `isComplete(template, filled)` сравнивает его с `template.cells`, которое отдаётся через `GET /colorings/:id`. | Можно отправить готовую карту, получить completion, artwork и server-side unlocks без прохождения игры. CAS защищает гонки, но не честность данных. |
