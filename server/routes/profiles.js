...
router.get('/:id/artworks', authMiddleware, async (req, res) => {
...
// POST /users/:id/add-stars  (debug endpoint)
router.post('/:id/add-stars', authMiddleware, async (req, res) => {
...
router.get('/collections/all', authMiddleware, async (req, res) => {
  const cols = await all('SELECT * FROM collections');
  res.json(cols);
});

// POST /collections/:id/add — add collection to user profile
router.post('/collections/:id/add', authMiddleware, async (req, res) => {
...
router.post('/artworks/:id/complete', authMiddleware, async (req, res) => {
