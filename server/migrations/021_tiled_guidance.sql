...
-- 021 adds compact global guidance data for the tiled player.
--
-- The full 1200x1200 grid is never materialized on the client or scanned
-- for navigation. Static per-tile/per-color counts are built once when a
-- tiled template is created; progress counters are bounded to affected
-- tiles/colors on each action. A missing progress row means the static
-- count is still fully remaining.
