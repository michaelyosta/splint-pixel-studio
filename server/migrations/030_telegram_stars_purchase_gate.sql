...
CREATE OR REPLACE FUNCTION prevent_telegram_stars_gate_audit_mutation()
RETURNS trigger LANGUAGE plpgsql AS $$
BEGIN
  RAISE EXCEPTION 'telegram_stars_purchase_gate_audit is append-only';
END;
$$;

CREATE TRIGGER trg_telegram_stars_gate_audit_no_update
  BEFORE UPDATE ON telegram_stars_purchase_gate_audit
  FOR EACH ROW EXECUTE FUNCTION prevent_telegram_stars_gate_audit_mutation();
