CREATE TABLE IF NOT EXISTS users (
  ...
  price_in_stars INTEGER NOT NULL DEFAULT 10 CHECK (price_in_stars >= 0),
...
CREATE TABLE IF NOT EXISTS collections (
  ...
  price_in_stars INTEGER NOT NULL DEFAULT 0 CHECK (price_in_stars >= 0),
  image_url TEXT
);
...
CREATE INDEX IF NOT EXISTS idx_artworks_owner ON artworks(owner_id, updated_at);
CREATE INDEX IF NOT EXISTS idx_posts_feed ON posts(status, published_at);
CREATE INDEX IF NOT EXISTS idx_posts_author ON posts(author_id, published_at);
