...
async function dropAllTables(pool) {
  await pool.query(`
    DROP TABLE IF EXISTS
      ...
      telegram_stars_capture_inbox,
      telegram_stars_purchase_gate_audit,
      telegram_stars_purchase_gate,
...
test('PostgreSQL runMigrations is idempotent', { skip: !databaseUrl }, async (t) => {
  ...
  assert.equal(result2.skipped, 37, 'Second run should skip all 37 migrations');
});
...
test('PostgreSQL schema_migrations contains correct versions and checksums', { skip: !databaseUrl }, async (t) => {
  ...
  assert.deepStrictEqual(versions, ['001', '002', '003', '004', '005', '006', '007', '008', '009', '010', '011', '012', '013', '014', '015', '016', '017', '018', '019', '020', '021', '022', '023', '024', '025', '026', '027', '028', '029', '030', '031', '032', '033', '034', '035', '036', '037'], 'Must contain exactly 001-037');
  assert.equal(checksums.length, versions.length, 'All migrations have checksums');
