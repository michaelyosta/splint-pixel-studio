...
test('Migration runner is idempotent', async (t) => {
  ...
  assert.equal(result2.skipped, 26, 'Second run should skip all migrations');
});
...
test('Known historical preview checksum remains upgrade-compatible', async () => {
  ...
  assert.equal(result.skipped, 26);
});
...
test('Legacy database (no schema_migrations) upgrades and applies post-baseline migrations', async (t) => {
  ...
  assert.equal(result.applied, 23, 'Legacy DB: should apply migrations 004 through 026');
  ...
  while (stmt.step()) versions.push(stmt.getAsObject().version);
  stmt.free();
  assert.deepStrictEqual(versions, ['001', '002', '003', '004', '005', '006', '007', '008', '009', '010', '011', '012', '013', '014', '015', '016', '017', '018', '019', '020', '021', '022', '023', '024', '025', '026'], 'All versions recorded');
