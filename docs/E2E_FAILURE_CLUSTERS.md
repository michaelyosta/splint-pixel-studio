...
Status: `CLUSTERED — bounded fixes integrated; selected final matrix green`
...
- The final selected matrix at code SHA `7d16ed3` is 16/16 shard green:
  `367 pass / 71 expected skip / 0 unexpected / 0 flaky`, retries `0`.
...
The original low-zoom assertion counted request starts, so cancelled requests
looked like excessive completed work. The corrected oracle counts completed
responses and waits for network idle. Stroke checks now use a causal target
fixture and geometric cell centers rather than a broad arbitrary coordinate
assumption. Repeated low-zoom and stroke evidence is green with retries `0`.
