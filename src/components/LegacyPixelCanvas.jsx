...
import { findForgivingCell } from '../lib/playLoop';

export default function PixelCanvas({
  template, filled, selectedColor, onPaint, onWrong, onFirstPaint, onTapCell,
  calmMode = false, hideFilledNumbers = true, hintMode = false, interactionMode = 'classic',
}) {
  ...
  useLayoutEffect(() => {
    ...
    const showNumbers = interactionMode === 'classic' && size >= 16;
    template.cells.forEach((targetColor, index) => {
      ...
      context.fillStyle = paint === -1 ? (interactionMode === 'reveal' ? '#17232d' : isSelected ? '#24465a' : isHint ? '#2f6f5a' : '#172735') : template.palette[paint];
    ...
    return undefined;
  }, [filled, selectedColor, template, wrongCell, hintMode, hideFilledNumbers, interactionMode]);

  function cellFromEvent(event) {
    const canvas = canvasRef.current;
    if (!canvas) return null;
    const rect = canvas.getBoundingClientRect();
    const gridX = ((event.clientX - rect.left) / rect.width) * template.width;
    const gridY = ((event.clientY - rect.top) / rect.height) * template.height;
    if (gridX < -0.2 || gridY < -0.2 || gridX > template.width + 0.2 || gridY > template.height + 0.2) return null;
    if (interactionMode === 'reveal') return findForgivingCell(template.width, template.height, filled, gridX, gridY);
    const x = Math.min(template.width - 1, Math.max(0, Math.floor(gridX)));
    const y = Math.min(template.height - 1, Math.max(0, Math.floor(gridY)));
